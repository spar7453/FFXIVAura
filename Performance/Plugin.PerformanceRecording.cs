...
public sealed unsafe partial class Plugin
{
        ...
        TooltipDiagnosticSnapshot tooltipDiagnostics)
    {
        ...
        this.AppendPerformanceProfileDiagnosticRow(builder, timestampUtc, "cache", "Cache", FormatDiagnosticPairs(
            ...
            ("partyRowBuffers", this.partyCooldownRowBuffersByWindow.Count),
            ("partyRowMemberBuffers", this.partyCooldownRowBuffersByWindow.Values.Sum(buffer => buffer.MemberBufferCount)),
        ...
        this.AppendPerformanceProfileDiagnosticRow(builder, timestampUtc, "aura", "Aura", FormatDiagnosticPairs(
            ...
            ("partyFallbackBatches", this.partyAuraStatusFallbackBatchCount),
            ("partyTimerRefreshAccepted", this.partyAuraTimerRefreshAcceptedCount),
            ("partyExpiredStatusSuppressed", this.partyAuraExpiredStatusSuppressedCount),
            ("partyTimerLastDecision", this.partyAuraTimerLastDecision),
            ...
            else if (string.Equals(observation.Result, "무시", StringComparison.Ordinal))
            {
                ...
                switch (observation.IgnoredReason)
                {
                    case PartyCooldownIgnoredLogReason.MemberNotFound:
                        ...
                        break;
                    case PartyCooldownIgnoredLogReason.OwnerNotFound:
                        partyLogOwnerNotFound++;
                        break;
                    ...
                    case PartyCooldownIgnoredLogReason.NotTrackedByWindow:
                        ...
                        break;
                    case PartyCooldownIgnoredLogReason.Ambiguous:
                        partyLogAmbiguous++;
                        break;
                    default:
                        partyLogOtherIgnored++;
                        break;
                }
            }
            else
        ...
        this.AppendPerformanceProfileDiagnosticRow(builder, timestampUtc, "partyCooldown", "Party Cooldown", FormatDiagnosticPairs(
            ...
            ("activeLiveStatuses", this.partyCooldownActiveStatusFrameCache.Values.Count(status => status.Origin == StatusSnapshotOrigin.Live)),
            ("activeFallbackStatuses", this.partyCooldownActiveStatusFrameCache.Values.Count(status => status.Origin == StatusSnapshotOrigin.Fallback)),
            ("activeAbsenceConfirmed", this.partyCooldownActiveStatusAbsenceConfirmed),
            ("liveStatusOwners", this.partyCooldownLiveStatusOwnerIdsBuffer.Count),
            ...
            ("rosterCacheHits", this.partyCooldownRosterCacheHitCount),
            ("rosterCacheMisses", this.partyCooldownRosterCacheMissCount),
        ...
        if (lastObservation is not null)
        {
            this.AppendPerformanceProfileDiagnosticRow(builder, timestampUtc, "partyCooldownLastLog", "Party Cooldown Last Log", FormatDiagnosticPairs(
                ...
                ("observedHudLocalAllianceGroupIndex", lastObservation.RosterDiagnostics.ObservedHudLocalAllianceGroupIndex),
                ("usedRetainedHudAllianceGroup", lastObservation.RosterDiagnostics.UsedRetainedHudAllianceGroup),
